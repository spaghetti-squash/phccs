...
function main() {
  ...
  smokeEmIfYouGotEm();
  if (!ignorewarnings) {
    var _iterator = _createForOfIteratorHelper(byAscendingStat(SPECIAL_REQUIREMENTS)),
      _step;
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var _step$value = _step.value,
          name = _step$value.name,
          reason = _step$value.reason,
          meets = _step$value.meets;
    ...
    if (args.softcore) {
      var missingItems = $items(_templateObject0 || (_templateObject0 = _taggedTemplateLiteral(["Stick-Knife of Loathing, Buddy Bjorn"]))).filter(i => !have$1(i));
  ...
  var pet = softcore ? byAscendingStat({
    Mysticality: $item(_templateObject1 || (_templateObject1 = _taggedTemplateLiteral(["astral chapeau"]))),
    Moxie: $item(_templateObject10 || (_templateObject10 = _taggedTemplateLiteral(["astral shorts"]))),
    Muscle: $item(_templateObject11 || (_templateObject11 = _taggedTemplateLiteral(["astral trousers"])))
  }) : $item(_templateObject12 || (_templateObject12 = _taggedTemplateLiteral(["astral statuette"])));
  kolmafia.visitUrl("council.php");
  ascend({
    path: $path(_templateObject13 || (_templateObject13 = _taggedTemplateLiteral(["Community Service"]))),
    playerClass: args.class,
    lifestyle,
    moon: "knoll",
    consumable: $item(_templateObject14 || (_templateObject14 = _taggedTemplateLiteral(["astral six-pack"]))),
    pet,
    permOptions: {
      neverAbort: true,
