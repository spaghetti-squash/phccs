...
const FAMILIAR_PICKS = [
  ...
  },
  {
    ...
    condition: () =>
      get("camelSpit") < 100 && !have($effect`Spit Upon`) && inHardcore(),
  },
  {
  ...
  },
  {
    ...
    condition: () => {
      ...
      if (
        !currentCupidFamiliar ||
        currentCupidFamiliar === $familiar`Cornbeefadon`
      )
        return true;
...
function chooseFamiliar(
  ...
  if (pick) {
    ...
    return {
      familiar: pick.familiar,
      famequip: ToyCupidBow.doneToday(pick.familiar)
        ? $item`tiny stillsuit`
        : $item`toy Cupid bow`,
    };
  }
...
export default function uniform({
  ...
  const spec: OutfitSpec = {
    ...DEFAULT_UNIFORM(),
    ...chooseFamiliar(canAttack),
    ...changes,
  };
  if (spec.familiar?.experience === 0) spec.beforeDress = [equipBluePlate];
